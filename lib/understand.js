...
export async function understand({ text = "", audioBase64 = "", mimeType = "audio/webm" } = {}) {
  ...
  const promptText = audioBase64
    ? `${RULES}\n\nThe attached audio is the person speaking in Bangla. It may be a ` +
      `low-quality, noisy 8kHz phone recording — listen carefully and transcribe ` +
      `as best you can into "transcript". Pay close attention to which basic need ` +
      `is mentioned (food/medicine/doctor/shelter/clothes/money) and do not ` +
      `confuse them; pick the intent that best matches what you actually hear.`
